...
struct node_cmp{
	...
	bool operator()(const node<T> *left,const node<T> *right) {
		return cmp_(left->key,right->key);
	}
private:
	Comparator cmp_;
};
...
public:
  ...
  void push(const T &key){
	...
	void push(Iterator begin, Iterator end) {
		std::vector<internal::node<T> *> new_elements;
		for (Iterator it = begin; it != end; ++it) {
			new_elements.push_back(new internal::node<T> (*it));
		}
		std::sort(new_elements.begin(),new_elements.end(),internal::node_cmp<T,Comparator>(cmp_));
		size_t n = new_elements.size();
		for(size_t i = n-1; i > 0; i--) {
			new_elements[i]->fix_leftlist();
			if( i%2 == 0 ) { //(i-1)/2 - parent of this element
				new_elements[(i-1)/2]->right_child = new_elements[i];
			}
			else{
				new_elements[(i-1)/2]->left_child = new_elements[i];
			}
		}
		new_elements[0]->fix_leftlist();
		const_node_ptr new_root;
		new_root = new_elements[0];
